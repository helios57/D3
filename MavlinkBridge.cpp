...
namespace d3 {
	...
	void MavlinkBridge::send_target(uint64_t timestamp, float x, float y) {
		target.timestamp = timestamp;
		target.x = x;
		target.y = y;
		sendTarget();
	}
	void MavlinkBridge::send_flow(uint64_t timestamp_from, uint64_t timestamp_to, float x, float y) {
